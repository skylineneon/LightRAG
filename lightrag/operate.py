...
def chunking_by_token_size(
    ...
    参数:
    tokenizer (Tokenizer): 用于将文本编码为令牌的分词器。
    content (str): 待分块的文本内容。
    split_by_character (str | None): 可选的分割字符，默认为 None。
    split_by_character_only (bool): 是否仅按分割字符分割，默认为 False。
    overlap_token_size (int): 分块间重叠的令牌数量，默认为 128。
    max_token_size (int): 每个分块的最大令牌数量，默认为 1024。

    返回:
    list[dict[str, Any]]: 包含分块信息的字典列表，每个字典包含 'tokens'、'content' 和 'chunk_order_index' 键。
    """
    # 将输入的文本内容编码为令牌列表
    ...
    if split_by_character:
        ...
        if split_by_character_only:
            for chunk in raw_chunks:
                ...
                # 将分块的令牌数量和内容作为元组添加到 new_chunks 列表中
                new_chunks.append((len(_tokens), chunk))
        else:
            for chunk in raw_chunks:
                _tokens = tokenizer.encode(chunk)
                if len(_tokens) > max_token_size:
                    # 若超过最大令牌限制，按最大令牌数量减去重叠令牌数量的步长进行遍历
                    for start in range(
                    ...
                    # 若当前文本块的令牌数量未超过最大令牌限制，直接将其令牌数量和文本内容作为元组添加到 new_chunks 列表
                    new_chunks.append((len(_tokens), chunk))
...
async def extract_entities(
    ...
    从文档的文本块中异步抽取实体和关系
    Args:
        chunks: 包含文本块的字典，键为块的唯一标识符，值为文本块的内容
        global_config: 全局配置字典，包含模型和其他参数
        pipeline_status: 管道状态字典，用于记录进度和状态
        pipeline_status_lock: 用于同步管道状态的锁
        llm_response_cache: LLM响应缓存，用于存储LLM的响应结果
    Returns:
        list: 包含抽取结果的列表，每个结果是一个元组 (maybe_nodes, maybe_edges)
    """
    use_llm_func: callable = global_config["llm_model_func"]
...
async def naive_query(
    ...
    参数：
        ...
        global_config: dict[str, str],  # 全局配置字典
        hashing_kv: BaseKVStorage | None = None,  # 用于缓存的键值数据库，默认值为None
        system_prompt: str | None = None  # 系统提示语
    返回：
        str | AsyncIterator[str]: 查询结果或异步迭代器
    """
    if query_param.model_func:
        use_model_func = query_param.model_func
    else:
